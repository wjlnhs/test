...
Grid.mixin({
    ...
    // 获取多人分享多人时列表颜色
    getMergeTable: function(event) {
       var html=[];
       if(event.IsMerge){
           var owner=event.OwnerIDs;
           html.push('<table class="mergetable" width="100%" cellpadding="0" cellspacing="0" ><tr>');
           for(var i=0;i<owner.length;i++){
               html.push('<td class="'+event._colorarr[owner[i]]+'"></td>')
           }
           html.push('</tr></table>');
       }
        return html.join('');;
...
DayGrid.mixin({
	...
	fgSegHtml: function(seg, disableResizing) {
        ...
        var tableHtml=this.getMergeTable(event);
		var timeHtml = '';
		var timeText;
		var titleHtml;
		...
		titleHtml =
			'<span title="'+htmlEscape(event.title || '')+'" class="fc-title">' +
				(htmlEscape(event.title || '') || '&nbsp;') + // we always want one line of height
			'</span>';
		
		return '<a class="' + classes.join(' ') + '"' +
				(event.url ?
					' href="' + htmlEscape('javascript:void(0)'||event.url) + '"' :
					''
					) +
				(skinCss ?
					' style="' + skinCss + '"' :
			...
			'>' +
            (tableHtml?tableHtml:'')+
				'<div title="'+htmlEscape(timeText||'')+' '+htmlEscape(event.title || '')+'" class="fc-content">' +
					(this.isRTL ?
	...
	renderSegPopoverContent: function(cell, segs) {
		...
		var content = $(
			'<div class="fc-header ' + view.widgetHeaderClass + '">' +
				...
				'"></span>' +
				'<span title="'+htmlEscape(title || '')+'" class="fc-title">' +
					htmlEscape(title) +
				'</span>' +
				'<div class="fc-clear"/>' +
			'</div>' +
            ...
            (tableHtml?tableHtml:'')+
				'<div title="'+htmlEscape(timeText||'')+' '+htmlEscape(event.title || '')+'" class="fc-content">' +
					(timeText ?
						...
						'>' +
							//'<span>' + htmlEscape(timeText) + '</span>' + //不显示时间
						'</div>' :
						''
						) +
					(event.title ?
						'<div title="'+htmlEscape(event.title || '')+'" class="fc-title">' +
...
function Calendar_constructor(element, overrides) {
	...
	function elementVisible() {
		return element.is(':visible')||element.css('display')=='block';//2015-4-21针对ie11修改
	}
