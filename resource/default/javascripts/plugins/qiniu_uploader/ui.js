...
function FileProgress(file, targetID) {
    ...
    if (!this.fileProgressWrapper.length) {
        ...
        var progressText = $("<p/>");
        progressText.addClass('title').text(file.name); //progressName


        var fileSize = plupload.formatSize(file.size).toUpperCase();
        var progressSize = $("<p/>");
        progressSize.addClass("imgWrap").addClass('progressName');//.html(fileSize);

        var progressBarTd = $('<span class="success" style="display: none;">完成</span>');

        var deleteBar = $('<div/>');
        deleteBar.addClass('file-panel');
        deleteBar.append('<span class="cancel">删除</span>');
        ...
        var progressBar = $('<div class="loading"><span class="loadingText">0%</span></div>');
      ...
      //  progressBarPercent.text(fileSize);
       ...
       // progressBarTd.append(progressBarBox);
        ...
        Wrappeer.append(deleteBar);
        Wrappeer.append(progressBarTd);
        Wrappeer.append(progressBar);
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    this.fileProgressWrapper.find('.loadingText').text(percentage);//.text("已上传: " + size + " 上传速度： " + formatSpeed + "/s");
...
FileProgress.prototype.setComplete = function(up, file, info) {
    ...
    this.fileProgressWrapper.attr('uuid',res.uuid);//设置上传完后的id
    if(!res.name){
        res.ext=file.name.substr(file.name.lastIndexOf('.')).toLowerCase();
    }
    res.name=res.name || file.name;
    ...
    var progressSuccess = this.fileProgressWrapper.find('.success');
    ...
    progressNameTd.data('qiniudata',JSON.stringify(res));
   ...
   // var showImg = $('<img src="loading.gif"/>');
    ...
    this.fileProgressWrapper.find('.loading').hide();

    if (!isImg) {
        var _fileName =  ((res||{}).key ||'');
        var _index = _fileName.lastIndexOf('.')||0;
        var _extention = _fileName.substr(_index+1);

        progressNameTd.append('<span class="i8files-ico-'+_extention+'"></span>');
       ...
       // linkWrapper.append(showImg);
        var showImg = $('<img style="width:64px;height:64px; vertical-align:middle;" src="'+i8_session.resHost+'default/images/64loading_white.gif"/>');
        progressNameTd.append(showImg);
        ...
        $(img).on('load', function() {
            showImg.attr('src', url).removeAttr('style');
...
FileProgress.prototype.appear = function() {
    ...
    this.fileProgressWrapper.addClass('complete');
    this.height = this.fileProgressWrapper.offset().top;
    this.opacity = 100;
