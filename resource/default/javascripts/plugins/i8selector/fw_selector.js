...
define(function (require, exports, module) {
    ...
    Array.prototype.getUniqueList=function(list){
        for(var i=0;i<list.length;i++){
            if(this.getIndex(list[i].OrgID)==-1&&this.getIndex(list[i].ID)==-1){
                this.push(list[i]);
            }
        }
        return this;
    ...
    var kSNSelector=function (setting) {
        //选人光标状态
        ...
        setting = $.extend({ model: 1,
            element: null,//初始化文本框ID
            width: 250,//选人控件初始化宽度
            maxSelected: 100,//最多能选择项数量
            isShowQuit: false,//是否显示离职人员
            readonly: false,//选人控件选项是否设置只读
            showIco: false,//老版兼容
            isAbox: false,//是否显示弹出框选择按钮
            selectPSNode:false,//当子父节点同时选择时，除去子节点，只保留父节点
            searchType: { "org": false, "user": true, "grp": false },//选择模式
            loadItem: {
                itemReadonly:false,
                items: [],
                loadedCallBack: function (obj) { }
            },//同控件初始化时加载
            selectCallback: function () { },//选择后回调
            deleteCallback: function () { },//删除项回调
            deleteBefore: function () { } },//删除已选择项之前回调
            setting);
        var singleObj = null;
        var multiObjs={};
        /*根据ID初始化控件*/
        var initSelector = function (elemid) {
            /*build(remove.start)*/
            if (setting.isAbox) {
                $.include(['default/javascripts/plugins/i8ztree/jquery.ztree.all-3.5.min.js', 'default/javascripts/plugins/i8ztree/treestyle.css']);
            }
            /*build(remove.end)*/
                ...
                if ($("#" + elemid).parents(".fw_ksntxtbox").length > 0 && $("#" + elemid).parents(".fw_ksninput").length > 0) {
                    ...
                    var selectorBox = $('<div class="fw_ksntxtbox" style="width:' + setting.width + (setting.width > 0 ? 'px;min-width:inherit' : '') + '"></div>').append(inputBox)
                        .click(function () {
                            ...
                            _this.find(".fw_ksnplaceholder").hide();
                        });
                    if ($("#" + elemid).attr("placeholder")) {
            ...
            var opts2 = {
                ...
                url: "/webajax/i8plugins/selector/selectentity?r=t&status="+(setting.isShowQuit?'2':'1'),
                model: setting.model,
            ...
            if(setting.searchType.org==false&&setting.searchType.grp==false){
                opts2.tipword="可以@同事";
            }
            ...
            if (setting.model == 0) {
                ...
                select2.deleteItemBefore = function (obj) {//删除之前
                    return setting.deleteBefore(obj, opts2.elem); /*obj:被删除的对象，opts.elem:原输入文本框对象*/
                }
                select2.seletedCallback = function (uid, uname, uemail, datatype,elem) {
                    return setting.selectCallback(uid, uname, uemail, datatype, elem); /*uid:选择用户ID，uname:选择用户名，uemail:选择用户邮箱,opts2.elem:输入原文本框对象*/
                }
            }
            ...
            $(opts2.elem).on('input propertychange',function () {
                var _this=$(this);
                var ksnSelector = _this.parents(".fw_ksntxtbox");
                if (ksnSelector.find(".fw_ksninput_slted").length >0||_this.val().length>0) {
                    ksnSelector.find(".fw_ksnplaceholder").hide();
                }
            });
            return select2;
        };
    ...
    (function ($) {
        $.extend({
            ...
            "detectAboxer": function (obj, setting) {
                ...
                var keybaordIco = $('<div class="fw_sboxer '+((tbset.org||tbset.grp)?"ksn-muti-ico":"fw_ksntxtbox_alert_ico")+'"></div>');
                keybaordIco.click(function () {
                    $(".fw_boxerContainer").remove(); $(".fw_agtlist").hide(); $("input", _ksnbox).val("");
                    var advBoxer = $('<div class="fw_boxerContainer"><div class="bc_loading"><img src="'+i8_session.resHost+'default/images/32loading_white.gif"/><span>加载中...</span></div></div>').click(function (e) { _ksnbox.find(".fw_ksnplaceholder").hide(); return false; });
                    ...
                    if (_left + 592 > _bodyWith) {/*当弹出的框超出body最右边时，将原位置向左移*/
                        _left = _bodyWith - 592 - 5;
                    }
                    ...
                    if (ksnSelector.find(".fw_ksninput_slted").length > 0) {
                        ...
                        ksnSelector.find(".fw_ksninput_slted").each(function () {
                            var jqItem = $.parseJSON(decodeURIComponent($(this).attr("f-data").replace(/undefined/g,false)));
                            var u_readonly=jqItem.ureadonly?jqItem.ureadonly:false;
                            var row_head='<img src="'+jqItem.headimg+'" class="sbox-usr-img"/>';
                            if(jqItem.datatype=="grp"){
                                row_head='<span class="ksn-org-grp-ico ksn-grp-pos"></span>';
                            }else if(jqItem.datatype=="org"){
                                row_head='<span class="ksn-org-grp-ico ksn-org-pos"></span>';
                            }
                            span_itemHtml += '<span class="item_slted" tagid="' + jqItem.uid + '" tagname="' + jqItem.uname + '" tagtype="' + jqItem.datatype + '" u-readonly="'+u_readonly+'" f-data="'+$(this).attr("f-data")+'">'+row_head+'<em>' + jqItem.uname + '</em>'+(jqItem.ureadonly?'':'<a></a>')+'</span>';
                        });
                    ...
                    $(".btn_submit_select", advBoxer).click(function () {
                        ...
                        if (_selecteditems.length == 0) {
                            ...
                            for (var i = 0; i < _selecteditems.length; i++) {
                                ...
                                var nodeItem={'id':jqItem.attr("tagid"),'type':jqItem.attr("tagtype"),'name':jqItem.attr("tagname"),readonly:jqItem.attr("u-readonly")=="true"};
                                span_itemHtml += "<span class=\"fw_ksninput_slted\" u-readonly=\""+nodeItem.readonly+"\" data-uid=\"" + nodeItem.id + "\" data-type=\"" + nodeItem.type + "\" f-data=\""+jqItem.attr("f-data")+"\"><em>" + nodeItem.name + "</em>"+(nodeItem.readonly?"":"<a></a>")+"</span>";
                            ...
                            _ksnbox.find("input").width(10)/*设置input宽度*/
                            advBoxer.remove();
                            ...
                            if (modelset == 1) {
                                ...
                                if (setting.selectCallback) { /*在单选情况下，依次返回uid, uname, uemail, datatype，elem*/
                                    ...
                                    setting.selectCallback(_selecteditems.attr("tagid"), _selecteditems.attr("tagname"), _selecteditems.attr("email"), _selecteditems.attr("tagtype"), _ksnbox.find("input")[0]);
                                }
                            } else {
                    ...
                    function convertTreeJson(source) {
                        ...
                        for (n in source) {
                            ...
                            tmp[item.OrgID].nocheck=!setting.searchType.org;
                            tmp[item.OrgID].checked=setting.selecteditems.getIndex(item.OrgID)>=0;
                    ...
                    var loadOrgTree = function () {
                        $.get("/webajax/i8plugins/selector/GetDefaultOrgTree", {}, function (response) {
                            if (response.Result) {
                                ...
                                var readonlyids=[];
                                $('#con_selectorItemList').find('.item_slted').each(function(i,item){
                                    var _item=$(item);
                                    var tagid=_item.attr('tagid');
                                    if(_item.attr('u-readonly')=='true'){
                                        readonlyids.push(tagid)
                                    }
                                    selecteditems.push({
                                        ID:_item.attr('tagid'),
                                        OrgID:_item.attr('tagid'),
                                        name:_item.attr('tagname'),
                                        nodeType:_item.attr('tagtype'),
                                        email:'',
                                        readonly:_item.attr('u-readonly'),
                                        headimg:_item.find('img').attr('src')
                                    });
                                });
                                var readonlyidstr=','+readonlyids.join(',')+',';
                                var tempdata = convertTreeJson(idObj,readonlyidstr);

                                var zTreeObj,
                                    _setting = {
                                        view: { selectedMulti: false,txtSelectedEnable:true, showIcon: showIconForTree, fontCss: setSpacialFontCss },
                                        check: { enable:setting.model==2},
                                        callback: {
                                            onExpand: function(event, treeId, treeNode){
                                                onExpandEvent(event, treeId, treeNode,readonlyidstr)
                                            },
                                            beforeClick: treeBeforeClick,
                                            onClick: nodeOnClickEvent,
                                ...
                                zTreeObj = $.fn.zTree.init($("#cropOrgTree"), _setting, tempdata);
                    ...
                    /*点击节点之前事件*/
                    var treeBeforeCheck = function (treeId, treeNode) {
                        var treeObj = $.fn.zTree.getZTreeObj(treeId);
                        if(setting.selectPSNode) {//启用了selectPSNode属性
                            var allChildNodes = treeObj.getNodesByParam("nodeType", "org", treeNode);
                            _.each(allChildNodes, function (item) {
                                treeObj.checkNode(item, !treeNode.checked);
                            })
                            if (treeNode.checked) {//取消
                                //var pnodes = treeNode.getParentNode();
                                cancelNode(treeNode);
                                function cancelNode(snode) {
                                    var pnd = snode.getParentNode();
                                    if (pnd) {
                                        treeObj.checkNode(pnd, false);
                                        cancelNode(pnd);
                                    }
                                }
                            } else {//选中
                                var pnd = treeNode.getParentNode();
                                if (pnd) {
                                    var allSCNodes = treeObj.getNodesByParam("nodeType", "org", pnd);
                                    var unchecked_nodes = _.where(allSCNodes, {checked: false});
                                    if (unchecked_nodes.length == 1) {
                                        //treeObj.checkNode(item,!treeNode.checked);
                                        pnd.checked = true;
                                    }
                                    console.log(allSCNodes);
                                }
                            }
                            treeObj.checkNode(treeNode, !treeNode.checked);
                           /* var _id = treeNode.nodeType == 'user' ? treeNode.ID : treeNode.OrgID;
                            var index = setting.selecteditems.getIndex(_id);
                            if (treeNode.checked) {
                                if (index == -1) {
                                    setting.selecteditems.push(treeNode);
                                }
                            } else {
                                setting.selecteditems.splice(index, 1);
                            }*/
                            setting.selecteditems=treeObj.getSelectedNodes();
                            nodeOnClickEvent(treeId, treeNode);
                            return false;
                        }else {
                            treeObj.checkNode(treeNode, !treeNode.checked);
                            var _id = treeNode.nodeType == 'user' ? treeNode.ID : treeNode.OrgID;
                    ...
                    var nodeOnClickEvent = function (event, treeId, treeNode, clickFlag) {
                        ...
                        var nodes =setting.model==2?treeObj.getCheckedNodes():treeObj.getSelectedNodes();
                        if (nodes.length > 0) {
                    ...
                    var loadGrpList = function () {
                        $.get("/webajax/i8plugins/selector/myGroupAndPublicParam?pageIndex=1&pageSize=0&queryType=1", {rad: Math.random() }, function (response) {
                            if (response.Result) {
                                var aList = response.List;
                                if (aList.length > 0) {
                                    var ulboxer = $('<ul id="ul_cropGroupList" class="crop_grouplist"></ul>');
                                    for (var i in aList) {
                                        var item=aList[i].Item1;
                                        if (item&&item.ID && item.Name) {
                                            var liRec = $('<li><div class="crop_grp_ico"><img src="'+item.Icon+'?imageView2/1/w/30/h/30" /></div><div class="crop_text"><span class="crop_grp_name_txt" headimg="'+item.Icon+'" dataid="' + item.ID + '">' + item.Name + '</span></div></li>').click(function () {
                                                if (!$(this).attr("class")) {
                                                    $(this).parent().find("li").removeClass("liselected");
                                                    $(this).addClass("liselected");
                                                    $("#btn_addselecteditemforls").removeClass("disable_add");
                                                    var _data = { "ID": $(".crop_grp_name_txt", this).attr("dataid"), "headimg": $.trim($(".crop_grp_name_txt", this).attr('headimg')), "name": $.trim($(".crop_grp_name_txt", this).text()), "nodeType": "grp" };
                    ...
                    /*展开*/
                    var onExpandEvent = function (event, treeId, treeNode,readonlyidstr) {
                        if (!treeNode.isExpand && treeNodeEnable.user) {
                            $.post('/webajax/i8plugins/selector/GetOrgPersonsInfo', {orgID:treeNode.OrgID,isOnlyContract:false,includeLowerOrgUser:false,pageSize:1000}, function (response) {
                                if (response.Result) {
                                    var treeObj = $.fn.zTree.getZTreeObj("cropOrgTree");
                                    ...
                                    if (Members.length == 0) {
                                        var newNodes = [{ "name": "无成员", "nodeType": "none", "noIco": true,nocheck:true}];
                                       ...
                                       /* var Persons = Members.sort(function (a, b) {
                                        ...
                                        for (var i = 0; i < Persons.length; i++) {
                                            var per = Persons[i];
                                            p_node.push({"name": per.Name + (per.Status == 1 ? "(已离职)" : ""), "ID": per.PassportID,"headimg":per.HeadImage, "nodeType": "user", "isManager":false /*per.IsManager*/, "email": per.Email,checked:setting.selecteditems.getIndex(per.PassportID)>=0 ,chkDisabled:(readonlyidstr.search(','+per.PassportID+',')>=0)});
                                        }
                    ...
                    $("#btn_addselecteditemforls", boxer_ctr).click(function () {
                        ...
                        if(setting.model==2){
                            $('#con_selectorItemList').find('.item_slted[tagtype=user]:not([u-readonly=true]),.item_slted[tagtype=org]:not([u-readonly=true])').remove();
                        }
                        var treeObj = $.fn.zTree.getZTreeObj("cropOrgTree");
                        var recNodes =setting.model==1?setting.selecteditems.getUniqueList(treeObj.getSelectedNodes()||[]):setting.selecteditems.getUniqueList(treeObj.getCheckedNodes()||[]);
                        //当子父节点同时选择时，除去子节点，只保留父节点
                        if(setting.selectPSNode) {
                            var fatNodes = _.where(recNodes, {"nodeType": "org"});
                            fatNodes = _.sortBy(fatNodes, "level");
                            for (var i = 0; i < fatNodes.length; i++) {
                                var currentNode = fatNodes[fatNodes.length - 1 - i];
                                var prntId = currentNode["parentTId"];
                                var prnds = _.where(fatNodes, {"tId": prntId});
                                if (prnds.length > 0) {
                                    currentNode['deprecated'] = true;
                                } else {
                                    currentNode["deprecated"] = false;
                                }
                            }
                            recNodes = _.where(fatNodes, {"deprecated": false});
                        }
                        var grpData=$(this).data();/*data数据目前只存放群组节点*/
                        if(grpData.name&&grpData.nodeType&&grpData.ID){/*群组数据拼装进去*/
                            recNodes.push({"name":grpData.name,"nodeType":grpData.nodeType,"ID":grpData.ID,headimg:grpData.headimg});
                        }
                        if (recNodes.length > 0) {
                            ...
                            for (var i = 0; i < recNodes.length; i++) {
                                ...
                                if (recNodes[i].nodeType == "user") {
                                    ...
                                    f_data.datatype="org";
                                    _category = "[组织]";
                                    recNodes[i].ID=recNodes[i].OrgID;
                                ...
                                var row_head='<img src="'+recNodes[i].headimg+'" class="sbox-usr-img"/>';
                                if(f_data.datatype=="grp"){
                                    //row_head='<span class="ksn-org-grp-ico ksn-grp-pos"></span>';
                                }else if(f_data.datatype=="org"){
                                    row_head='<span class="ksn-org-grp-ico ksn-org-pos"></span>';
                                }
                                var sp_sltItem = $('<span class="item_slted" tagid="' + recNodes[i].ID + '" tagname="' + recNodes[i].name + '" tagtype="' + recNodes[i].nodeType + '" u-readonly="false" email="' + recNodes[i].email + '" f-data="'+encodeURIComponent($.JSONString(f_data))+'">'+row_head+'<em>' + _category + recNodes[i].name + '</em><a></a></span>');
                                if (modelset == 1 && $("#con_selectorItemList span.item_slted").length >0) {
                                    alert('单选框模式只能待选一项');
                    ...
                    $(".bccr_selectedItem", boxer_ctr).bind("click", function (event) {
                        if ($(event.target)[0].tagName == "A") {
                            var treeObj = $.fn.zTree.getZTreeObj("cropOrgTree");
                            var item_slted=$(event.target).parents(".item_slted");
                            var _id=item_slted.attr('tagid');
                            setting.selecteditems.splice(setting.selecteditems.getIndex(_id),1);
                            var node=item_slted.attr('tagtype')=='org'?treeObj.getNodesByParam('OrgID',_id,null):treeObj.getNodesByParam('ID',_id,null);
                            if(node&&node.length>0){
                                treeObj.checkNode(node[0],false);
                            }
                            item_slted.remove();
                            addSelectedItemBarScroll();
                        }
    ...
    function KSNSelect(opts) {
        ...
        this.deleteItemBefore = function (data) { }; //删除选项之前操作
        this.tWarp = opts.tArea || "tWarp";  //定位框
        this.tipword=opts.tipword||"可以@同事、部门、群组";
    }
    ...
    KSNSelect.prototype = {
        ...
        add: function (elem, txtData, nStart, nLen, guid,email,dtype) {//插入文本   参数  操作的元素，数据，起始坐标位置，用户输入字符长度
            if (!$(this.elem).is(":hidden")&&!$(this.elem).attr("disabled")) {
                this.elem.focus();
            }
            var _range;
                        ...
                        success: function (data) {
                            ...
                            if (data.Result) {
                                ...
                                if (dataList.length > 0) {
                                    $("#" + _sugListID).show().html("<dl><dt>"+_this.tipword+"</dt>" + dataList + "</dl>")
        ...
        KIInit: function () {/*绑定*/
            ...
            fw_ksntxtbox.bind("click", function (event) {
                if (_this.deleteItemBefore) {
                    if(_this.deleteItemBefore($(event.target).parent()[0])){
                        return false;
                    }
                }
                if ($(event.target)[0].tagName == "A") {
        ...
        KIshow: function (event) {/*显示*/
            ...
            if (_inputData.length > 0) {
                ...
                $.ajax({
                    url: this.url + "&k=" + encodeURIComponent(_inputData) + "&g=" + _guidslist + "&t=" + _this.sType,
                    type: "get",
                    dataType: "json",
                    data: {},
                    success: function (rqData) {
                        if (rqData.Result) {//数据请求成功
                            var tempdata = rqData.ReturnObject; //数据列表
                ...
                if (_inputData.length == 0 && event.keyCode == 8) {
                    ...
                    if (t_length == 1) {
                        ...
                        var preKsnItem=$(this.elem).prev(".fw_ksninput_slted");
                        if(preKsnItem.attr("u-readonly")=="false"|| preKsnItem.attr("u-readonly")=="undefined"){
                            preKsnItem.remove();
                        }
        ...
        addin: function (sltData,addtype) {
            ...
            if (_this.seletedCallback != null && addtype == "SELECT_ADD") {
                if (_this.seletedCallback(uid, uname, uemail, datatype,_this.elem) == "cancel") {/*当selectedCallback返回'cancel'串时，选不中*/
                    go = false;
                }
            }
        },
        ...
        KISetSelectedData: function (data, _callback) { /*设置或加载人、群组、组织，使之已选中【新增方法】*/
            ...
            if (data && data.length > 0) {
                ...
                $.post('/webajax/i8plugins/selector/getpers?', { data: loadinfos }, function (response) {
                    if (response) {
                        if (response.Result) {
                            var datalist = response.ReturnObject;
                            for (var i = 0; i < datalist.length; i++) {
                                var _u_readonly= (_.findWhere(data,{id:datalist[i].id.toString()}) && _.findWhere(data,{id:datalist[i].id.toString()}).ureadonly) || false;
    ...
    exports.KSNSelector=kSNSelector;
})