...
//        },/// 交易账号
    ...
    var saveCompanyThirdStep=function(isValid,step,isDraft){
        ...
        var modelUpload={
            modelpairMult:[{
                key:'BankCopyUrl',
                text:'近6个月公司或个人银行对账单（法人、法人配偶、股东等可合并计算，尽量多提供）'
            }]
        }
        getModelAndSave(modelUpload,isValid,step,isDraft);
    }
    ...
    var saveCompanyFifthStep=function(isValid,step,isDraft){
        ...
        var modelUpload={
            modelpair:[{
                key:'MarriageCertificateUrl',
                text:'借款人结婚证'
            },{
                key:'MarriageCertificate2Url',
                text:'借款人配偶结婚证'
            },{
                key:'AccountBookUrl',
                text:'户口本(整本)'
            },{
                key:'SpouseIdentityCardUrl',
                text:'借款人配偶身份证'
            },{
        ...
        var validateimg=function(model,step){
            if(isValid){
                for(var i=0;i<modelpair.length;i++){
                    if(!model[modelpair[i].key] && !modelpair[i].isnotRequire){
        ...
        var validateimgMult=function(model,step){
            if(isValid){
                for(var i=0;i<modelpairMult.length;i++){
                    ...
                    if(_.isEmpty(obj) && !modelpairMult[i].isnotRequire){
            ...
            ajax.postData({
                ...
                for(var i=0;i<modelpairMult.length;i++){
                    ...
                    if($.isEmptyObject(curFiles)){
                        curFiles={Name:modelpairMult[i].key};
    ...
    var bindCompanyInfo=function(data){
        if($.type(data)=='object'&&data.Result){
            if(data.ReturnObject){
                ...
                window.location.hash="apply_page"+((data.ReturnObject.Step>6?6:data.ReturnObject.Step)||1)+'&'+(_data.ReturnObject.ApplyID || _data.ReturnObject.ID);
