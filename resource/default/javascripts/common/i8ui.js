...
define(function(require,exports){
    ...
    * */
    ...
    var render_loading=function(box,type){
        ...
        var $box=$(box)
        if($box.get(0).tagName.toLowerCase()=='tbody'){
            $box.html('<td colspan="999"><div class="ld-64-'+type+'" style="width:64px;height: 64px;"></div></td>');
        }else{
            $box.html('<div class="ld-64-'+type+'" style="width:64px;height: 64px;"></div>');
        }
        return
    }
    var render_no_data=function(box,txt){
        //没有数据
        var txt=txt || "没有找到任何数据~";
        var $box=$(box)
        if($box.get(0).tagName.toLowerCase()=='tbody'){
            $box.html('<td colspan="999"><div class="noresult">\
